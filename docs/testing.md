...
- **API:** `/api/replays` (filtros, orden, paginación, detalle con ambos equipos, 404, PATCH favorito, `/status`),
  ...
  `/summary`, `/me`, `/api/players/{name}/summary` (with/against), `/ballchasing` y `/frames` (mockeados),
  `/api/settings` (GET shape + known_players; PUT re-tag de `is_me` y recálculo de V/D al cambiar de jugador;
  validación de vacíos).
- **Parser real** (`subtr_actor` mockeado vía `sys.modules`, sin binario): `parser.parse_replay` happy path
...
- **Frames 3D** (`replay_frames.py`, `rrrocket` mockeado): helpers puros (`_is_ball/_is_car/_uid_key/
  ...
  ok/returncode/JSON inválido/timeout/exe ausente) y `get_frames_cached` (caché válida/inválida/miss en tmp).
  **replay_frames.py ~84%.**
- **Stats avanzadas** (`advanced_stats.compute_advanced`, puro): posesión (coche más cercano al balón),
  ...
  y el mapeo idx→PlayerStat (`_assign_advanced_to_stats`): match exacto, asignación por eliminación
  cuando el candidato es único, y **anti-swap** (dos compañeros sin emparejar → NULL, no se cruzan).
- **Mapa de tiros** (`shot_map.compute_shots`, puro): tiro a gol on-target + velocidad, parada (toque
  `save` rival en ventana), fuera (tiro abierto/off-target), toques no-tiro ignorados; y endpoint
  `/api/replays/{id}/shots` (mapeo player_id→nombre por platform_id, caché, no_local_replay).
...
(`make_replay`/`make_player` + builders `build_subtr_*`/`build_rrrocket_data`),
`backend/tests/fixtures/tracker_sample.json`.
