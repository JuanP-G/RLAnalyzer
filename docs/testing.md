...
Infra: `backend/conftest.py` (fixtures `engine`/`db`/`client` + `_patch_sessionlocal` para la BD de test),
`backend/tests/factories.py` (`make_replay`/`make_player`), `backend/tests/fixtures/tracker_sample.json`.
